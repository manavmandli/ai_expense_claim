frappe.ui.form.on("Expense Claim", {
	custom_upload_process_aibased_bills(frm) {
		show_ai_bill_processor(frm);
	}
});

function show_ai_bill_processor(frm) {
	let files = [];
	...
	async function show_results_dialog(groups) {
		const review = new frappe.ui.Dialog({
			...
			primary_action: async function() {
				...
				try {
					...
					if (frm.doc.name) {
						const all_file_names = selected
							.flatMap(r => r._file_doc_names || [])
							.filter(Boolean);
