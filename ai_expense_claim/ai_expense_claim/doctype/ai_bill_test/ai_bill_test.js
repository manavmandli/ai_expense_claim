// Copyright (c) 2026, AI Expense Claim and contributors
// For license information, please see license.txt

frappe.ui.form.on("AI Bill Test", {
	refresh(frm) {
		if (frm.fields_dict.upload_bills_html) {
			frm.fields_dict.upload_bills_html.$wrapper.html(`
				<div style="margin-bottom: 16px;">
					<button class="btn btn-primary btn-sm" id="upload_bills_btn">
						${frappe.utils.icon("upload", "sm")}
						<span style="margin-left: 6px;">${__("Upload & Process Bills")}</span>
					</button>
					<p style="margin-top: 8px; font-size: 12px; color: var(--text-muted);">
						${__("Upload one or multiple bills to process with AI")}
					</p>
				</div>
			`);
			
			frm.fields_dict.upload_bills_html.$wrapper.find("#upload_bills_btn").on("click", () => {
				show_multi_bill_processor(frm);
			});
		}
		
		if (frm.doc.status === "Success") {
			frm.dashboard.set_headline_alert(
				`<div class="text-success">✓ Confidence: ${frm.doc.confidence || 0}%</div>`
			);
		} else if (frm.doc.status === "Low Confidence") {
			frm.dashboard.set_headline_alert(
				`<div class="text-warning">⚠ Low Confidence: ${frm.doc.confidence || 0}%</div>`
			);
		} else if (frm.doc.status === "Failed") {
			frm.dashboard.set_headline_alert(
				`<div class="text-danger">✗ Processing Failed</div>`
			);
		}
	}
});

function show_multi_bill_processor(frm) {
	let files = [];
	let processing = false;

	const dialog = new frappe.ui.Dialog({
		title: __("Upload & Process Bills"),
		size: "large",
		fields: [
			{
				fieldname: "upload_html",
				fieldtype: "HTML"
			}
		],
		primary_action_label: __("Process with AI"),
		primary_action: process_files,
		secondary_action_label: __("Cancel"),
		secondary_action: () => dialog.hide()
	});

	dialog.show();
	
	dialog.$wrapper.find('.modal-body').css({
		'max-height': '70vh',
		'overflow-y': 'auto'
	});
	
	render_upload_ui();
	update_button_state();

	if (!document.getElementById("ai-upload-styles-test")) {
		inject_styles();
	}

	function inject_styles() {
		const style = document.createElement("style");
		style.id = "ai-upload-styles-test";
		style.textContent = `
			.ai-dropzone-test {
				border: 2px dashed var(--border-color);
				border-radius: var(--border-radius-lg);
				padding: 48px 20px;
				text-align: center;
				cursor: pointer;
				transition: all 0.3s;
				background: var(--bg-light-gray);
				margin-bottom: 16px;
			}
			.ai-dropzone-test:hover {
				border-color: var(--primary);
				background: var(--blue-50, #eff6ff);
			}
			.ai-dropzone-test.dragover {
				border-color: var(--primary);
				background: var(--blue-100, #dbeafe);
				transform: scale(1.01);
			}
			.ai-drop-icon {
				font-size: 48px;
				color: var(--text-muted);
				margin-bottom: 12px;
			}
			.ai-drop-label {
				font-size: 15px;
				font-weight: 500;
				margin: 0 0 6px;
				color: var(--text-color);
			}
			.ai-drop-hint {
				font-size: 13px;
				color: var(--text-muted);
				margin: 0;
			}
			.ai-file-list-test {
				display: flex;
				flex-direction: column;
				gap: 8px;
				margin-top: 16px;
			}
			.ai-file-item-test {
				display: flex;
				align-items: center;
				gap: 12px;
				padding: 10px 12px;
				border: 1px solid var(--border-color);
				border-radius: var(--border-radius);
				background: var(--card-bg);
				transition: all 0.2s;
			}
			.ai-file-item-test:hover {
				background: var(--bg-light-gray);
			}
			.ai-drag-handle-test {
				cursor: grab;
				color: var(--text-muted);
				flex-shrink: 0;
			}
			.ai-drag-handle-test:active {
				cursor: grabbing;
			}
			.ai-file-thumb-test {
				width: 48px;
				height: 48px;
				border-radius: var(--border-radius-sm);
				overflow: hidden;
				flex-shrink: 0;
				background: var(--bg-light-gray);
				display: flex;
				align-items: center;
				justify-content: center;
			}
			.ai-file-thumb-test img {
				width: 100%;
				height: 100%;
				object-fit: cover;
			}
			.ai-file-info-test {
				flex: 1;
				min-width: 0;
			}
			.ai-file-name-test {
				font-size: 13px;
				font-weight: 500;
				overflow: hidden;
				text-overflow: ellipsis;
				white-space: nowrap;
			}
			.ai-file-size-test {
				font-size: 12px;
				color: var(--text-muted);
			}
			.ai-file-actions-test {
				display: flex;
				gap: 6px;
				flex-shrink: 0;
			}
			.ai-list-header-test {
				display: flex;
				align-items: center;
				justify-content: space-between;
				margin: 16px 0 8px;
				font-size: 12px;
				color: var(--text-muted);
			}
		`;
		document.head.appendChild(style);
	}

	function render_upload_ui() {
		const wrapper = dialog.fields_dict.upload_html.$wrapper;
		
		wrapper.html(`
			<input type="file" id="ai_file_input_test" multiple accept="image/*,.pdf" style="display:none">
			<div class="ai-dropzone-test" id="ai_dropzone_test">
				<div class="ai-drop-icon">${frappe.utils.icon("upload", "xl")}</div>
				<p class="ai-drop-label">${__("Click or drop files here")}</p>
				<p class="ai-drop-hint">${__("Supports PDF and images (JPG, PNG, WEBP)")}</p>
			</div>
			<div id="ai_file_list_container_test"></div>
		`);

		const dropzone = wrapper.find("#ai_dropzone_test");
		const file_input = wrapper.find("#ai_file_input_test");

		dropzone.on("click", (e) => {
			e.preventDefault();
			file_input[0].click();
		});

		file_input.on("change", (e) => {
			add_files(Array.from(e.target.files));
			e.target.value = "";
		});

		dropzone.on("dragover", (e) => {
			e.preventDefault();
			e.stopPropagation();
			dropzone.addClass("dragover");
		});

		dropzone.on("dragleave", (e) => {
			e.preventDefault();
			e.stopPropagation();
			dropzone.removeClass("dragover");
		});

		dropzone.on("drop", (e) => {
			e.preventDefault();
			e.stopPropagation();
			dropzone.removeClass("dragover");
			add_files(Array.from(e.originalEvent.dataTransfer.files));
		});
	}

	function add_files(new_files) {
		const allowed = ["image/jpeg", "image/png", "image/webp", "application/pdf"];

		new_files.forEach(file => {
			if (!allowed.includes(file.type)) {
				frappe.show_alert({
					message: __('"{0}" is not supported. Use PDF or image files.', [file.name]),
					indicator: "orange"
				});
				return;
			}

			if (files.find(f => f.name === file.name && f.size === file.size)) {
				return;
			}

			if (file.type.startsWith("image/")) {
				const reader = new FileReader();
				reader.onload = (e) => {
					file.preview = e.target.result;
					render_file_list();
				};
				reader.readAsDataURL(file);
			}

			files.push(file);
		});

		render_file_list();
		update_button_state();
	}

	function render_file_list() {
		const wrapper = dialog.fields_dict.upload_html.$wrapper;
		const container = wrapper.find("#ai_file_list_container_test");

		if (!files.length) {
			container.html("");
			return;
		}

		let html = `
			<div class="ai-list-header-test">
				<span>${__("Files to process")}</span>
				<span class="badge badge-pill badge-primary">${files.length}</span>
			</div>
			<div class="ai-file-list-test" id="ai_sortable_list_test">
		`;

		files.forEach((file, idx) => {
			const size = format_file_size(file.size);
			const thumb = file.preview
				? `<img src="${file.preview}" alt="${frappe.utils.escape_html(file.name)}">`
				: `<div style="color: var(--text-muted);">${frappe.utils.icon("file", "md")}</div>`;

			html += `
				<div class="ai-file-item-test" data-idx="${idx}">
					<div class="ai-drag-handle-test">${frappe.utils.icon("drag", "sm")}</div>
					<div class="ai-file-thumb-test">${thumb}</div>
					<div class="ai-file-info-test">
						<div class="ai-file-name-test" title="${frappe.utils.escape_html(file.name)}">${frappe.utils.escape_html(file.name)}</div>
						<div class="ai-file-size-test">${size}</div>
					</div>
					<div class="ai-file-actions-test">
						<button class="btn btn-xs btn-default ai-remove-btn-test" data-idx="${idx}" title="${__("Remove")}">
							${frappe.utils.icon("delete", "sm")}
						</button>
					</div>
				</div>
			`;
		});

		html += `</div>`;
		container.html(html);

		container.find(".ai-remove-btn-test").on("click", function() {
			const idx = $(this).data("idx");
			files.splice(idx, 1);
			render_file_list();
			update_button_state();
		});

		init_sortable();
	}

	function init_sortable() {
		const list_el = dialog.fields_dict.upload_html.$wrapper.find("#ai_sortable_list_test")[0];
		if (window.Sortable && list_el && files.length > 1) {
			new window.Sortable(list_el, {
				animation: 150,
				handle: ".ai-drag-handle-test",
				onEnd(evt) {
					const moved = files.splice(evt.oldIndex, 1)[0];
					files.splice(evt.newIndex, 0, moved);
				}
			});
		}
	}

	function format_file_size(bytes) {
		if (bytes >= 1048576) return (bytes / 1048576).toFixed(1) + " MB";
		if (bytes >= 1024) return (bytes / 1024).toFixed(1) + " KB";
		return bytes + " B";
	}

	function update_button_state() {
		dialog.get_primary_btn().prop("disabled", files.length === 0 || processing);
	}

	function process_files() {
		if (processing || !files.length) return;

		processing = true;
		dialog.hide();

		Promise.all(files.map(file_to_base64))
			.then(file_data => {
				return frappe.call({
					method: "ai_expense_claim.ai_expense_claim.doctype.ai_bill_test.ai_bill_test.process_multiple_bills_test",
					args: { files: JSON.stringify(file_data) },
					freeze: true,
					freeze_message: __("AI is processing your bills...")
				});
			})
			.then(r => {
				const response = r.message || {};
				const groups = response.groups || [];
				const low_confidence_files = response.low_confidence_files || [];
				const total = response.total_processed || 0;
				
				if (!groups.length && !low_confidence_files.length) {
					frappe.throw(__("No data could be extracted from the files."));
				}
				
				// Show warnings for low confidence files
				if (low_confidence_files.length > 0) {
					const warnings = low_confidence_files.map(f => 
						`<li><strong>${f.file}</strong>: ${f.reason} (${f.confidence}% confidence)</li>`
					).join("");
					frappe.msgprint({
						title: __("Some Files Skipped"),
						message: `
							<p>${__("The following files were skipped due to low confidence:")}</p>
							<ul>${warnings}</ul>
						`,
						indicator: "orange"
					});
				}
				
				if (groups.length > 0) {
					show_results_dialog(groups, total);
				}
			})
			.catch(err => {
				dialog.show();
				frappe.msgprint({
					title: __("Error"),
					message: err.message || __("Failed to process bills"),
					indicator: "red"
				});
			})
			.finally(() => {
				processing = false;
			});
	}

	async function show_results_dialog(groups, total) {
		const review = new frappe.ui.Dialog({
			title: __("Review Extracted Expenses"),
			size: "extra-large",
			fields: [
				{
					fieldname: "expenses",
					fieldtype: "Table",
					label: __("Expenses"),
					options: "AI Bill Test Detail",
					cannot_add_rows: true,
					cannot_delete_rows: true,
					in_place_edit: true,
					fields: [
						{ fieldname: "expense_date", fieldtype: "Date", label: __("Date"), in_list_view: 1, reqd: 1, columns: 1 },
						{ fieldname: "expense_type", fieldtype: "Data", label: __("Type"), in_list_view: 1, columns: 2 },
						{ fieldname: "amount", fieldtype: "Currency", label: __("Amount"), in_list_view: 1, columns: 1 },
						{ fieldname: "description", fieldtype: "Small Text", label: __("Description"), in_list_view: 1, columns: 3 },
						{ fieldname: "custom_attachment", fieldtype: "Attach", label: __("Attachment"), in_list_view: 1, read_only: 1, columns: 2 },
						{ fieldname: "confidence", fieldtype: "Percent", label: __("Confidence"), in_list_view: 1, read_only: 1, columns: 1 },
					],
				}
			],
			primary_action_label: __("Apply to Form"),
			primary_action: async function() {
				const selected = grid.get_selected_children();

				if (!selected.length) {
					frappe.show_alert({ message: __("Select at least one expense to apply."), indicator: "orange" });
					return;
				}

				const apply_btn = review.get_primary_btn();
				apply_btn.prop("disabled", true).text(__("Applying..."));

				try {
					for (const row of selected) {
						// Check for existing row with same date + expense type
						const existing = (frm.doc.expenses || []).find(
							e => e.expense_date === row.expense_date && e.expense_type === row.expense_type
						);

						if (existing) {
							// Append to existing row
							existing.amount = parseFloat(existing.amount || 0) + parseFloat(row.amount || 0);
							if (row.description) {
								existing.description = [existing.description, row.description]
									.filter(Boolean).join("; ");
							}
							if (row.custom_attachment) {
								if (existing.custom_attachment) {
									const r = await frappe.call({
										method: "ai_expense_claim.api.expense.merge_attachments_by_url",
										args: { url1: existing.custom_attachment, url2: row.custom_attachment }
									});
									if (r.message) existing.custom_attachment = r.message;
								} else {
									existing.custom_attachment = row.custom_attachment;
								}
							}
						} else {
							// Add new child row
							const child = frm.add_child("expenses");
							child.expense_date = row.expense_date;
							child.expense_type = row.expense_type;
							child.amount = row.amount;
							child.description = row.description;
							child.confidence = row.confidence;
							if (row.custom_attachment) child.custom_attachment = row.custom_attachment;
						}
					}

					frm.refresh_field("expenses");

					// Update overall form fields with summary
					if (selected.length > 0) {
						const first = selected[0];
						frm.set_value("confidence", first.confidence);
						frm.set_value("status", first.confidence >= 95 ? "Success" : "Low Confidence");
					}
					
					// Save extracted data as JSON
					frm.set_value("extracted_data", JSON.stringify(groups, null, 2));

					if (frm.doc.name) {
						await frm.save();
					}

					review.hide();
					frappe.show_alert({
						message: __("{0} expense item(s) applied", [selected.length]),
						indicator: "green"
					});
				} catch(err) {
					console.error(err);
					frappe.show_alert({ message: __("Failed to apply expenses. Please try again."), indicator: "red" });
				} finally {
					apply_btn.prop("disabled", false).text(__("Apply to Form"));
				}
			},
			secondary_action_label: __("Cancel"),
			secondary_action: () => review.hide()
		});

		review.show();

		const grid = review.fields_dict.expenses.grid;
		grid.df.data = groups.map((g, idx) => ({
			name: frappe.utils.get_random(10),
			idx: idx + 1,
			doctype: "AI Bill Test Detail",
			__checked: 1,
			expense_date: g.expense_date,
			expense_type: g.expense_type,
			amount: g.amount,
			description: g.description,
			custom_attachment: g.attachment_url || "",
			confidence: g.avg_confidence || 0,
			_file_doc_names: g.file_doc_names || [],
		}));
		grid.refresh();
	
		requestAnimationFrame(() => {
			// Pre-select all rows
			grid.grid_rows.forEach(row => {
				if (row.doc) row.doc.__checked = 1;
			});
			grid.wrapper.find(".grid-row-check").prop("checked", true);
		});
	}

	function file_to_base64(file) {
		return new Promise((resolve) => {
			const reader = new FileReader();
			reader.onload = (e) => resolve({
				name: file.name,
				data: e.target.result,
				type: file.type
			});
			reader.readAsDataURL(file);
		});
	}
}
